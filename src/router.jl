...
"""
    Router(objective, strategies, n_tokens)

Constructs a router that finds a set of trades `(router.Δs, router.Λs)` through `strategies` 
which maximizes `objective`. The number of tokens `n_tokens` must be specified.
"""
function Router(objective::O, strategies::Vector{C}, n_tokens) where {T, O<:Objective, C<:Numo{T}}
...
Router(objective, n_tokens) = Router(objective, Vector{Numo{Float64}}(), n_tokens)

function find_arb!(r::Router, v)
    Threads.@threads for i in 1:length(r.Δs)
        find_arb!(r.Δs[i], r.Λs[i], r.strategies[i], v[r.strategies[i].Ai])
    end
end
...
function route!(r::R; v=nothing, verbose=false, m=5, factr=1e1, pgtol=1e-5, maxfun=15_000, maxiter=15_000) where {R<:Router}
    ...
    function fn(v)
        ...
        for (Δ, Λ, c) in zip(r.Δs, r.Λs, r.strategies)
